...
def Liste_des_clients_non_rentables(df):
    ...
    if days == 7:
        st.write(f"{len( df[df['activity_per_day_7_days'] < 1.25][['full_name.1', 'activity_per_day_7_days']].sort_values(by=['activity_per_day_7_days'], ascending=False).rename(columns={'full_name.1' : 'Nom du client','activity_per_day_7_days': 'Nombre moyen séance par jours sur les 7 derniers jours'}))} clients sur {len(df)} ne sont pas rentables sur les 7 derniers jours")
        ...
        st.write(f"{len( df[df['activity_per_day_year'] < 1.25][['full_name.1', 'activity_per_day_year']].sort_values(by=['activity_per_day_year'], ascending=False).rename(columns={'full_name.1' : 'Nom du client','activity_per_day_year': 'Nombre moyen séance par jours sur 1 an'}))} clients sur {len(df)} ne sont pas rentables sur 1 an")
